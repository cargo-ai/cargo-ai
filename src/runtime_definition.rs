...
impl RuntimeAgentDefinition {
    ...
    pub(crate) fn validate_provider_output(&self, raw: &str) -> Result<Value, String> {
        let parsed = serde_json::from_str::<Value>(raw)
            .map_err(|_| "The provider returned output that could not be parsed as JSON.".to_string())?;
    ...
    fn validate_response_value(&self, value: &Value) -> Result<(), String> {
        ...
        for property in &self.schema_properties {
            let field_value = object.get(property.name.as_str()).ok_or_else(|| {
                format!("Missing required field `{}` in provider output.", property.name)
            })?;
...
fn parse_action_execution(root_obj: &Map<String, Value>) -> Result<crate::ActionExecutionMode, String> {
...
fn parse_runtime_vars(root_obj: &Map<String, Value>) -> Result<Vec<crate::RuntimeVarSpec>, String> {
    ...
    for (name, raw_spec) in runtime_vars {
        validate_flat_identifier(name, format!("$.runtime_vars.{name}").as_str(), "runtime variable")?;
...
fn parse_runtime_var_default(
    ...
    validate_runtime_var_default(field_type, default_value, format!("{path}.default").as_str())?;
...
fn parse_enum_values(
    ...
    if *field_type != SchemaFieldType::String {
        return Err(format!("{path}.enum: `enum` is only supported for `string` fields"));
    }
...
fn parse_numeric_constraints(
    ...
    if !matches!(field_type, SchemaFieldType::Number | SchemaFieldType::Integer) {
...
fn parse_actions(root_obj: &Map<String, Value>) -> Result<Vec<crate::Action>, String> {
    ...
    if actions.is_empty() {
        return Err("$.actions: must contain at least one action".to_string());
    }
    ...
    match kind.as_str() {
        ...
        "agent" => {
            ...
            Ok(crate::RunStep {
                ...
                subject: None,
                text: None,
                agent: Some(required_non_empty_string(run_obj, "agent", path)?),
...
fn required_run_args(run_obj: &Map<String, Value>, path: &str) -> Result<Vec<crate::RunArg>, String> {
...
fn optional_logic_object(
    ...
    if !value.is_object() {
        return Err(format!(
            "{path}.{field_name}: expected a JSON Logic object"
        ));
    }
    Ok(Some(value.clone()))
}
...
fn optional_action_input_overrides(
    ...
    for (name, entry_value) in overrides {
        validate_flat_identifier(name, format!("{path}.input_overrides.{name}").as_str(), "named input")?;
...
fn optional_action_run_vars(
    ...
    for (name, entry_value) in run_vars {
        validate_flat_identifier(name, format!("{path}.run_vars.{name}").as_str(), "runtime variable")?;
...
fn optional_action_inputs(
    ...
    let inputs = value
        .as_array()
        .ok_or_else(|| format!("{path}.inputs: expected `inputs` to be an array of ordered input parts"))?;
    if inputs.is_empty() {
        return Err(format!("{path}.inputs: must contain at least one child input"));
    }
...
fn optional_platforms(
    ...
    for platform in values {
        ...
        if normalized.is_empty() {
            return Err(format!("{path}.platform: expected a non-empty platform value"));
        }
...
fn optional_named_identifier(
    ...
    let name = value
        ...
        .trim()
        .to_string();
    validate_flat_identifier(name.as_str(), format!("{path}.{field_name}").as_str(), "named input")?;
    Ok(Some(name))
}
...
fn optional_owned_path(
    ...
    if validate_file_extension {
        validate_supported_file_extension(raw_path.as_str(), format!("{path}.{field_name}").as_str())?;
    }
    Ok(Some(raw_path))
}

fn validate_definition_owned_local_path(raw_path: &str, path: &str, label: &str) -> Result<(), String> {
    if raw_path.trim().is_empty() {
        return Err(format!("{path}: {label} path must be a non-empty relative path"));
    }
...
fn validate_supported_file_extension(raw_path: &str, path: &str) -> Result<(), String> {
    ...
    match extension.as_deref() {
        ...
        _ => Err(format!("{path}: file input path must use a supported extension")),
    }
}
...
fn is_valid_schema_version(value: &str) -> bool {
    ...
    revision.parse::<u32>().ok().filter(|number| *number > 0).is_some()
}
...
#[cfg(test)]
mod tests {
    use super::RuntimeAgentDefinition;
