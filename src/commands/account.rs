...
use crate::config::adder::{set_account_email, set_account_tokens};
    ...
    if let Some(reg_m) = sub_m.subcommand_matches("register") {
        ...
        if let Some(test_m) = mail_m.subcommand_matches("test") {
            ...
            let acct = match cfg.account.as_ref() {
                ...
                None => {
                    eprintln!(
                        "❌ No account found in config. You must confirm your account first."
                    );
                    return;
                }
            };
        ...
        let agents_command = if let Some(list_m) = agents_m.subcommand_matches("list") {
            ...
            let definition_json =
                match serde_json::from_str::<serde_json::Value>(&definition_json_raw) {
                    Ok(v) => v,
                    Err(e) => {
        ...
        } = &agents_command
        {
            ...
            if is_pull_success {
                ...
                super::hatch_pipeline::run_hatch_pipeline(name, definition_json_str);
                return;
            }
        }
