...
Starting with [Cargo AI 0.4.3](../releases/0.4.3.md), the CLI and newly generated agents automatically retain metadata under `usage/usage.sqlite3` in this same selected home. Existing generated binaries need rebuilding to gain this behavior. Check [release availability](testing-and-release-qualification.md#release-status) for the published version. The embedded SQLite engine requires no runtime installation; source builds need the supported C toolchain. Profiles remain in `config.toml` and credentials remain in their existing credential backend. Keep the live database on a local filesystem, outside network shares and cloud file sync.
