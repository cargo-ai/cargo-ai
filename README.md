...
Cargo AI prints one run-level mode header before actions start. When output is redirected, piped, or running in simpler terminals, it prefixes parent-visible action output with deterministic lane labels such as `[A1 first_action]`, and long-running steps emit a step-start liveness line such as `step 2/2 generate_image started; waiting for provider response...`. When attached directly to an interactive terminal, it switches to a compact live lane dashboard that groups each action lane by label, running status with elapsed time, terminal step marker/current step, last lifecycle message, and a compact buffered `output` section. Parent-run `exec` and `email_me` output is bucketed into that lane, while child-agent steps stay minimal in the parent view with start/completion or exit summaries instead of recursively inlining the child transcript.
